...
function Color() {
  ...
  function Checkbox() {
    ...
    function onChangeHandler(position, event) {
      ...
      if (updatedIsChecked[position]) {
        ...
        setActiveColors((previous) =>
          previous.filter((color) => color !== event.target.name)
        );
    ...
    useEffect(() => {
      if (!query.get("color"))
        dispatch(filterActions.updateColor({ colors: noFilter.current }));
    ...
    return (
      <>
        {colors.map((color, index) => (
          <div className={classes.container} key={`color${index}`}>
