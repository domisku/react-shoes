...
function Size() {
    ...
    function Checkbox() {
        ...
        const [isChecked, setIsChecked] = useState(new Array(sizes.length).fill(false));
        ...
        useEffect(() => {
            ...
            sizes.forEach((size, index) => {
              if (queryValues.includes(size.toString())) updatedIsChecked[index] = true;
            });
          ...
          function onChangeHandler(position, event) {
            ...
            function deleteQuery(name) {
              const sizeQueries = query.getAll('size').filter(size => size !== name);
              query.delete('size');
        ...
        useEffect(() => {
            if (!query.get('size')) dispatch(filterActions.updateSize({sizes: noFilter.current}));
            else dispatch(filterActions.updateSize({sizes: activeSizes.map(size => +size)}));
        }, [activeSizes]);

        return (
            <>
                {sizes.map((size, index) => <div className={classes.container}>
                    <label htmlFor={`size${index}`}>
                        {size}
                        <input key={index} name={size} type='checkbox' id={`size${index}`} checked={isChecked[index]} onChange={(event) => onChangeHandler(index, event)}></input>
