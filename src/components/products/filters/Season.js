...
function Season() {
    ...
    function Checkbox() {
        ...
        const [isChecked, setIsChecked] = useState(new Array(seasons.length).fill(false));
        ...
        useEffect(() => {
          ...
          function onChangeHandler(position, event) {
            ...
            function deleteQuery(name) {
              const seasonQueries = query.getAll('season').filter(season => season !== name);
              query.delete('season');
              for (let seasonQuery of seasonQueries) query.append('season', seasonQuery);
            }
        ...
        useEffect(() => {
            if (!query.get('season')) dispatch(filterActions.updateSeason({seasons: noFilter.current}));
            else dispatch(filterActions.updateSeason({seasons: activeSeasons}));
        }, [activeSeasons]);

        return (
            <>
                {seasons.map((season, index) => <div className={classes.container}>
                    <label htmlFor={`season${index}`}>
                        {season}
                        <input key={index} name={season} type='checkbox' id={`season${index}`} checked={isChecked[index]} onChange={(event) => onChangeHandler(index, event)}></input>
