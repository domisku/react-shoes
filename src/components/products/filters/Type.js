...
function Type() {
    ...
    function Checkbox() {
        ...
        const [isChecked, setIsChecked] = useState(new Array(types.length).fill(false));
        ...
        useEffect(() => {
          ...
          function onChangeHandler(position, event) {
            ...
            function deleteQuery(name) {
              const typeQueries = query.getAll('type').filter(type => type !== name);
              query.delete('type');
        ...
        useEffect(() => {
            if (!query.get('type')) dispatch(filterActions.updateType({types: noFilter.current}));
            else dispatch(filterActions.updateType({types: activeTypes}));
        }, [activeTypes]);

        return (
            <>
                {types.map((type, index) => <div className={classes.container}>
                    <label htmlFor={`type${index}`}>
                        {type}
                        <input key={index} name={type} type='checkbox' id={`type${index}`} checked={isChecked[index]} onChange={(event) => onChangeHandler(index, event)}></input>
