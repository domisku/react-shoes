...
function Cart() {
  ...
  async function removeFromCart(event) {
    ...
    try {
      ...
      setFilteredData(
        filteredData.filter(
          (product) =>
            product.size !== cartData[keyInDatabase].size ||
    ...
    try {
      await fetch(
        `https://react-shoes-default-rtdb.europe-west1.firebasedatabase.app/users/${username}/cart/${keyInDatabase}.json?auth=${idToken}`,
        {
          ...
          body: JSON.stringify({
            quantity:
              event.target.title === "Add"
                ? quantities[event.target.name] + 1
                : quantities[event.target.name] - 1,
          }),
        }
      );
      ...
      if (event.target.title === "Add") newQuantities[event.target.name]++;
        ...
        newQuantities[event.target.name]--;
  ...
  return (
    <>
      ...
      {cartData && isLoggedIn && (
        <div className={classes.container}>
          {filteredData.map((product, index) => (
            <div key={`cart-item${index}`} className={classes.wrapper}>
                ...
                <div className={classes["change-quantity-container"]}>
                  <button
                    className={`${classes["decrease-quantity"]} ${
                      quantities[index] <= 1 &&
                      classes["decrease-quantity-disabled"]
                    }`}
                    title="Remove"
          ...
          <div className={classes.total}>
            <h3>Total: </h3>
            {filteredData.reduce((acc, cur, index) => {
              return acc + cur.price * quantities[index];
            }, 0)}{" "}
            &euro;
          </div>
          {!DBisEmpty && !toCheckout && (
            <button
          ...
          {toCheckout && (
            <Checkout closeCheckout={() => setToCheckout(false)} />
          )}
        </div>
      )}
      {isLoading && (
      ...
      {DBisEmpty && (
        <div className={classes.container}>
          <span className={classes["empty-list"]}>Your cart is empty.</span>
        </div>
      )}
      {!isLoggedIn && (
        <div className={classes.container}>
          <span className={classes["empty-list"]}>
            You must be logged in to see your cart items.
          </span>
        </div>
      )}
    </>
